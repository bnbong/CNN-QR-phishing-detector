...
P0 ~ P3 전체 실험과 2차 실험(motif·절제·프로브·외부 검증 F·캠페인 홀드아웃 G)을 마쳤다. F 중 F-c(역방향 전이)와 F-d(혼합 학습)는 미실행이다.

| 연구 질문 | 현재 답 |
|---|---|
| RQ1. 길이·버전·패딩 shortcut을 통제한 뒤에도 QR 모듈 격자에 판별 신호가 있는가 | 그렇다. 캠페인·템플릿 단위 홀드아웃에서도 유지된다 |
...
| RQ3. 그 신호가 여러 피싱 QR에 반복되는 국소 motif로 나타나며 새 도메인·데이터셋에서도 재현되는가 | 앞부분은 그렇다. 뒷부분은 절반이다 — 판별 신호는 약하게 전이되지만 개별 motif 정체는 재현되지 않는다 |
...
국소 motif는 존재하고 조성으로 환원되지 않는다. 3×3 패치 히스토그램만으로 돌린 LR이 0.740/0.707/0.715로 라벨 셔플 바닥(0.49 ~ 0.53)을 뚜렷이 넘고, QR 안에서 모듈 위치만 섞으면 0.498/0.502/0.510으로 바닥까지 무너지며 바이트 조성을 보존하는 코드워드 셔플에서도 0.547/0.539/0.555에 그친다. 다만 개별 motif의 odds ratio는 1.4 ~ 1.6배 수준이고, CNN과의 갭은 3×3 창보다 넓은 범위의 배치·순서에서 온다.

motif는 CNN 판단의 원인이기도 하다. 매칭된 3×3 창의 중심 모듈 하나만 뒤집는 motif 표적 중심 비트 개입(motif-targeted center-bit intervention)을 걸면 AUROC가 −0.016/−0.025/−0.037 떨어지는데, 개수뿐 아니라 위치·국소 밀도까지 맞춘 무작위 대조는 −0.002 ~ −0.008에 그치고 로짓도 방향에 맞게 움직인다. 다만 절제로 사라지는 몫은 CNN과 바닥 사이 격차의 수 %p다.

"CNN이 URL 어휘를 복원한다"는 주장은 철회한다. 임베딩을 얼리고 붙인 선형 프로브에서 유의한 목표가 v2 2/91(둘 다 경로 구조 특징), v3·v4는 0개다. 라벨 자체는 0.872/0.813/0.804로 잘 분리되지만 개별 n-gram·키워드·숫자 비율은 미학습 CNN 임베딩 기준선조차 넘지 못한다. CNN이 쓰는 것은 특정 문자열이 아니라 문자 조성과 국소 비트 패턴 수준의 통계로 보인다.

**외부 검증(F).** WebPhish 모델을 2026년 OpenPhish + Common Crawl 외부 세트에 그대로 걸면 AUROC가 0.744/0.702/0.693으로, 그룹 순열 바닥선(0.54 ~ 0.58)은 넘지만 내부 대비 0.11 ~ 0.13 떨어져 판정이 `reproduced_weak`다. 같은 외부 데이터로 학습하면 0.87 ~ 0.93이 나오므로 신호가 없어서가 아니라 전이가 안 되는 것이고, char n-gram LR(0.81 ~ 0.87)이 CNN보다 잘 전이된다. 개별 motif 재현은 실패다(Spearman ρ 0.09 ~ 0.43, 상위 20 Jaccard 귀무 수준). 분포 수준 신호는 전이되나 개별 motif 정체는 데이터셋 특이적이다. 인기 도메인 confound와 benign 정제 낙관 편향은 민감도 세 조건에서 모두 관측되지 않았다.

**템플릿 누출(G).** 고정 캠페인 test 위의 쌍체 ΔAUROC(A_sm−B)가 −0.010/+0.001/−0.014로 세 층 모두 CI가 0을 포함하고 순열 p도 0.50 ~ 0.95라, phishing kit·경로 템플릿 누출은 무시 가능하다. 옛 비쌍체 G가 보던 큰 차이는 test 집합이 교체된 효과였다.

따라서 "피싱 QR에 보편적인 시각 signature가 있다"는 문장은 현재 자료로 쓸 수 없다. 지지되는 것은 피싱 URL의 문자 조성 차이가 QR 모듈 공간에 학습 가능한 공간 신호로 나타난다는 데까지다.
...
독립 결과 보고서(논문 초안의 뼈대)는 [`docs/REPORT.md`](docs/REPORT.md), 전체 표와 해석은 [`docs/RESULTS.md`](docs/RESULTS.md).
