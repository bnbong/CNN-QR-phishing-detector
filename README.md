...
대상은 **정규화된 URL 문자열에서 합성한 QR 격자**다. 촬영·인쇄된 실제 QR 이미지는 범위 밖이다. 디코딩 없이 QR 구조·픽셀 패턴으로 quishing을 탐지하는 접근은 이미 선행연구가 있으므로(Trad & Chehab, arXiv:2505.03451) 이 저장소의 기여는 "처음 제안"이 아니라 **성능을 길이·생성 조건·공간 배치·국소 패턴·외부 전이로 분해해 그것이 보편적 시각 서명인지 평가하는 것**이다.
...
QR은 URL이 길어지면 버전이 올라가 격자가 커진다. 남는 데이터 코드워드는 `0xEC/0x11` 교대 패딩으로 채워지므로 **패딩 시작 위치가 URL 길이를 그대로 누출한다.** 두 클래스의 길이 분포가 다른 데이터셋에서는 이 하나만으로도 정확도가 천장을 친다.
...
| RQ2. CNN은 명시적 디코딩 없이 공간 배치를 이용해 URL 유래 내용·순서 신호(URL-derived content/order signal)에 접근하는가 | 접근한다. 다만 사용한 선형 프로브와 판정 기준으로는 학습에 의한 어휘 접근성 향상 증거를 확보하지 못했다 |
| RQ3. 그 신호가 여러 피싱 QR에 반복되는 국소 motif로 나타나며 새 도메인·데이터셋에서도 재현되는가 | 앞부분은 그렇다(절제 설계 개정 후 재실행 전까지 잠정). 뒷부분은 절반이다 — 판별 신호는 바닥선 위에 남지만 개별 motif 정체는 재현되지 않는다 |
...
motif는 CNN 판단의 원인으로 보인다. 매칭된 3×3 창의 중심 모듈 하나만 뒤집는 motif 표적 중심 비트 개입(motif-targeted center-bit intervention)을 걸면 AUROC가 −0.016/−0.025/−0.037 떨어지는데, 개수뿐 아니라 위치·국소 밀도까지 맞춘 무작위 대조는 −0.002 ~ −0.008에 그치고 로짓도 방향에 맞게 움직인다. 다만 절제로 사라지는 몫은 CNN과 바닥 사이 격차의 수 %p이고, 두 개입의 차이에 대한 쌍체 CI는 아직 없다. motif 선정을 시드별 train으로 옮기고 무작위 대조를 반복별 AUROC로 바꾸는 재설계를 마쳤으므로 위 수치는 재실행 전까지 잠정이다.

"CNN이 URL 어휘를 복원한다"는 주장은 쓰지 않는다. 임베딩을 얼리고 붙인 선형 프로브에서 어휘 정보 자체는 v2 45/91이 셔플 기준선을 넘어 접근 가능했지만, 미학습 CNN 임베딩 기준선까지 넘어 "학습이 접근성을 높였다"고 말할 수 있는 목표는 v2 2개(둘 다 경로 구조 특징), v3·v4는 0개다. 그 정보가 실제 분류 결정에 쓰이는지는 동결 표현 위의 선형 프로브로 측정할 수 없다. 정리하면 사용한 프로브와 기준으로는 복원 증거를 확보하지 못했다는 데까지다. CNN이 무엇을 쓰는지는 확정할 수 없다.

**외부 검증(F).** WebPhish 모델을 2026년 OpenPhish + Common Crawl 외부 세트에 그대로 걸면 AUROC가 0.744/0.702/0.693으로, 그룹 블록 교환 근사 바닥선(0.54 ~ 0.58)은 넘지만 내부 대비 0.11 ~ 0.13 떨어진다. 같은 외부 데이터로 학습하면 0.87 ~ 0.93이 나오므로 신호가 없어서가 아니라 전이가 안 되는 것이다. char n-gram LR(0.81 ~ 0.87)이 CNN보다 높지만 이 비교는 첫 시드만 쓴 기준선과 5시드 CNN을 맞댄 것이라 잠정이다. 개별 motif 재현은 실패다(Spearman ρ 0.09 ~ 0.43, 상위 20 Jaccard 귀무 수준). 인기 도메인 confound와 benign 정제 낙관 편향은 민감도 세 조건에서 관측되지 않았다.

여기서 **출처 편향에 대한 옛 서술을 정정한다.** "WebPhish는 benign이 짧고 경로가 없는데 외부는 방향이 반대"라고 적었던 것은 틀렸다. 실측하면 경로 보유율은 WebPhish benign 99.7% 대 phishing 69.8%, 외부 benign 93.9% 대 phishing 46.0%로 두 데이터셋이 같은 방향의 편향을 공유한다. 게이트를 차단이 아니라 플래그 방식으로 재설계했고(길이+경로 동시 매칭 cohort가 주 판정, 경로 모양만 쓰는 `path_lr` 기준선이 0.80 이상이면 서술 전용으로 강등), 표의 `reproduced_weak` 판정은 재실행 전까지 유효하지 않다.

**템플릿 누출(G).** 고정 캠페인 test 위의 쌍체 ΔAUROC(A_sm−B)가 −0.010/+0.001/−0.014이고 CI 상한이 0.0011/0.0145/0.0151로 사전에 정한 SESOI 0.02 아래다. 즉 정의한 템플릿 분리 방식과 평가 집합에서 전체 AUROC 상승을 SESOI 아래로 한정했다. 다만 누출 기회가 실제로 있는 행(시드당 38/35/15)만 보면 CI 상한이 0.09 ~ 0.16이라 검출도 배제도 못했다. phishing kit 암기 가능성을 배제한 것은 아니다. 옛 비쌍체 G가 보던 큰 차이에 대해서는, 그 비교가 test 교체 효과와 누출 효과를 분리할 수 없었다는 데까지만 말할 수 있다.

따라서 "피싱 QR에 보편적인 시각 signature가 있다"는 문장은 현재 자료로 쓸 수 없다. 지지되는 것은 QR 모듈 격자의 **원래 배치에 분류에 유용한 구조가 있다**는 데까지이며, 그 구조가 무엇으로 이루어졌는지는 확정할 수 없다.

길이 아티팩트는 작았다. 길이 주변분포를 클래스 간 일치시킨 주 조건과, 통제를 완전히 푼 `L-none`의 차이가 +0.005/+0.002/+0.024에 그친다(길이가 다른 특징과 상호작용해 만드는 효과까지 없앤 것은 아니다). 이는 버전 층화와 정규화가 이전 실험의 누출을 이미 대부분 걷어냈다는 것을 뜻한다. 반대로 정규화 자체의 효과는 크다. `raw`는 `norm`보다 0.05 ~ 0.09 높다. 초기 실험은 지표와 조건이 달라 수치를 직접 견줄 수 없지만, 표면 정규화가 걷어내는 몫이 이만큼이라는 사실은 그때 검증하기 어려웠던 부분을 가리킨다.
