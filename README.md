...
**CNN 표현에서 주소 문자 정보를 읽을 수 있는지는** 프로브 집계를 바로잡은 뒤 다시 답해야 한다. 지금까지 확인된 것은 학습이 경로 구조 두 가지(경로 유무, 경로 깊이)에 대한 접근성을 높였다는 것까지이며, 개별 문자·단어 조각의 판정은 집계 오류 때문에 아직 확정되지 않았다.
...
| CNN 표현에서 주소 문자를 읽어낼 수 있는가 | 집계를 바로잡은 뒤 다시 답해야 한다 |
