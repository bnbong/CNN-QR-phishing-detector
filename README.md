...
이 실험은 그 질문에서 출발했다. QR 코드는 URL을 손실 없이 그대로 담고 있다. 그러나 정보가 보존된다는 사실만으로 "피싱 사이트임을 맞힐 수 있는가"가 자동으로 풀리지는 않는다. 그 정보가 피싱 여부를 얼마나 예측하는지, 그리고 작은 CNN이 코드를 읽지 않고도 그것을 배울 수 있는지를 실험으로 확인하려고 했다. 
...
**CNN은 이 데이터의 QR을 보고** 정상·피싱 라벨을 어느 정도 구분한다. QR 코드 크기별로 집단을 나누고, 각 집단에서 피싱과 정상의 주소 길이 분포를 1바이트 단위까지 똑같이 맞춘 뒤에도 성능이 남는다. 길이 통제를 완전히 풀었을 때와의 차이는 0.005 ~ 0.024 AUROC.

**원래 QR의 배치는** 이 CNN이 학습하는 데 도움이 된다. 칸의 값은 그대로 두고 위치만 뒤섞으면 CNN 성능이 0.11 ~ 0.15 떨어지는데, 위치를 보지 않는 대조 모델은 거의 변하지 않는다.

**그렇다고 모든 피싱 QR에 통하는** 특정 무늬를 찾지는 못했다. 작은 무늬 조각의 빈도에 클래스와 연관된 구조가 있는 것까지는 확인했지만, 그 조각 하나하나가 피싱의 서명이라는 근거는 얻지 못했다.

**특정 무늬를 골라 훼손했을 때의** 특별한 효과도 확실하게 확인되지 않았다. 고른 무늬를 겨냥해 칸을 뒤집는 개입과, 같은 개수, 비슷한 위치의 칸을 아무렇게나 뒤집는 개입의 차이가 대조 조건과 시드에 걸쳐 일관되지 않았다.

**새 출처에서도 약한 구분력은 남지만,** 주소 문자를 직접 분석한 모델이 피싱 사이트 구분을 더 잘한다. 학습에 쓴 데이터와 완전히 다른 출처의 2026년 데이터에 그대로 걸면 성능이 0.649 / 0.639 / 0.677로 떨어진다. 우연 수준(0.53 ~ 0.56)보다는 높으니 신호가 사라진 것은 아니지만 같은 조건에서 주소 텍스트를 그냥 읽는 방식이 여전히 더 잘 맞힌다.

**CNN 표현에서 주소 문자 정보를** 선형으로 읽어낼 수 있는 목표는 v2에서 약 29%(26/91), v3 11%(13/114), v4 1%(1/104)다. 그중 학습으로 접근성이 높아진 것은 v2의 경로 구조 두 가지(경로 유무, 경로 깊이)뿐이다. 즉 문자 정보가 일부 선형으로 존재하기는 하지만 대부분은 학습하지 않은 CNN에서도 똑같이 읽히는 입력 구조에서 오고, 피싱 분류 학습이 읽기 쉽게 만든 부분은 경로 구조에 한정된다. 그 정보를 모델이 실제 판단에 쓰는지는 측정하지 못했다.
...
| CNN 표현에서 주소 문자를 읽어낼 수 있는가 | 일부만. 선형으로 읽히는 목표가 26/91, 13/114, 1/104이고 학습이 접근성을 높인 것은 v2의 경로 구조 두 개뿐 |

## 이 연구가 말하지 않는 것

- 입력은 QR 촬영본이 아닌 위에서 설명한 합성 격자이며, 촬영 잡음 등을 배제하여 오로지 QR에만 집중한 실험이다.
- 주소를 소문자로 바꾸고 끝의 `/`를 떼는 등 표기를 통일한 상태로 실험했다. URL을 QR로 변환할 때 '/' 등의 작은 문자 요소 차이가 QR 무늬에 변화를 주기 때문.
- 학습은 사실상 한 데이터셋에 기대며, 여러 출처를 아우른 결론이 아니다.
- 외부 데이터 비교에서 맞춘 것은 주소 길이와 "경로가 있는가" 두 가지의 결합 분포까지이며, 경로 깊이나 구분자 구성까지 맞춘 것은 아니다. 매칭 전후로 평가 대상 표본 자체가 달라지므로 두 점수의 차이를 경로 편향의 기여량으로 읽을 수 없다.
...
- [`docs/review_01.md`](docs/review_01.md), [`docs/review_02.md`](docs/review_02.md), [`docs/review_03.md`](docs/review_03.md), [`docs/review_04.md`](docs/review_04.md) - 외부 리뷰와 그에 따른 수정 이력
