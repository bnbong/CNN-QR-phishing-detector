...
이 실험으로 확인하고 싶은 것:
1. 합성곱 모델이 **디코딩 없이** 모듈 배치만 보고 URL 유래 신호를 어디까지 활용하는가. 
...
| RQ2. CNN은 명시적 디코딩 없이 공간 배치를 이용해 URL 유래 어휘·순서 신호에 접근하는가 | 접근한다. 다만 그 신호는 개별 어휘 특징으로 선형 분해되지 않는다 |
...
char n-gram LR과의 갭은 0.10 ~ 0.19다. 이 기준선은 강한 텍스트 베이스라인이지 Bayes 최적 분류기가 아니므로 상한선이 아니고, CNN이 넘더라도 누출의 증거가 되지 않는다. 갭의 크기는 "남은 정보량"이 아니라 이 기준선과의 거리다. 그래도 디코딩 없이 모듈 배치만으로 URL 유래 신호의 상당 부분에 접근한다는 것은 분명하다.
...
motif는 CNN 판단의 원인이기는 하다. 상위 motif가 걸린 모듈을 뒤집으면 AUROC가 −0.016/−0.025/−0.037 떨어지는데, 같은 개수의 무작위 모듈을 뒤집을 때는 −0.002 ~ −0.005에 그치고 로짓도 방향에 맞게 움직인다. 다만 절제로 사라지는 몫은 CNN과 바닥 사이 격차의 수 %p다.
