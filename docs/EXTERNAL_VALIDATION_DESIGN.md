...
6. 바닥선: **라벨 셔플 대신 점수 순열 검정**을 쓴다. 모델이 고정이므로 라벨을 재학습 없이 섞을 수 있다.
   ...
   **순열 횟수.** 설계 목표는 2,000회이지만 지금까지 실행된 값은 **200회**다(결과 JSON의 `n_perm`).
   문서와 표에는 실행값을 적고, 재실행에서 2,000회로 올린다.
...
**표본 수를 표에 병기한다.** F-a의 CNN 평가와 motif 히스토그램 기준선은 지금까지 같은 층에서도
서로 다른 행 수로 계산됐다(외부 v2 기준 CNN 15,480행 대 motif 기준선 8,000행). 동일 cohort 비교를
추가하거나, 그것이 불가능하면 표에 두 표본 수를 나란히 적는다. 표본 수가 다른 두 숫자를 한 칸에
나란히 놓고 크기를 비교해서는 안 된다.

**기준선도 5시드 쌍체로 낸다.** 현재 표의 외부 기준선 수치는 **첫 성공 시드 하나에서만** 계산된 값이고
CNN 쪽은 5시드 평균이다. 재실행에서는 시드별 학습 자료와 test를 맞춘 뒤 5시드 쌍체 Δ와 그 CI로
교체한다. 그전까지 "CNN보다 얼마나 높다"는 비교는 잠정이다.
...
> **현재 표에 실린 `reproduced_weak` 판정은 옛 게이트 기준으로 계산된 값이다.** 재설계된 게이트와
> `a_fixed_lenpath` cohort, `path_lr` 기준선이 들어간 Colab 재실행 전까지 **유효하지 않다.**
> 재실행 후의 판정으로 교체한다.
