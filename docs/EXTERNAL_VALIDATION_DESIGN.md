...
`reports/external/bias_diagnostics.json`. 주 세트(EXT-P + Common Crawl benign) 기준:

| 클래스 | n | `path_depth>=1` | 바이트 길이 중앙값 (p10/p90) | top5 그룹 점유율 |
|---|---|---|---|---|
| benign | 60,562 | **0.940** | 51 (25 / 104) | 0.001 |
| phishing | 555,853 | **0.761** | 46 (18 / 132) | 0.089 |

benign 경로 보유율이 phishing보다 **높다**. WebPhish의 편향축(benign 짧고 경로 없음 / phishing 긺)과
방향이 반대이므로 게이트를 통과했다. 스킴 보유율은 양쪽 0.0으로 규약대로 제거됐다.
EXT-B2(Tranco)는 설계대로 경로 보유율 0.0이고, 이는 대조군의 정의 자체다.
...
| 그 motif가 CNN 판단에 인과적으로 기여한다 | 지지, 효과는 작음 (5-A절) |
...
Tranco 맨 도메인만 쓰면 "benign은 짧고 경로가 없다"는 WebPhish의 편향을 그대로 재현하게 되고,
...
즉 편향축의 부호가 WebPhish와 반대이거나 최소한 무관하다. 여기서 WebPhish 학습 모델의 AUROC가
유지되면 신호는 출처 지문이 아니고, 무너지면 1차 결론의 상당 부분이 출처 지문이었다는 뜻이다.
...
6. 바닥선: **라벨 셔플 대신 점수 순열 검정**을 쓴다. 모델이 고정이므로 라벨을 재학습 없이 섞을 수 있다.
   외부 라벨을 **그룹 단위로** 2,000회 순열해 얻은 AUROC 분포의 97.5 백분위를 `null_ci_upper`로 기록한다.
   행 단위 순열은 그룹 내 라벨 상관을 깨서 바닥선을 낮게 잡으므로 쓰지 않는다.
...
- `length LR` 전이 AUROC ∈ [0.48, 0.52], `version LR` ∈ [0.48, 0.52]
- `bias_diagnostics.path_depth_ge1_frac`이 WebPhish와 **같은 방향의 편향이 아님**
- 외부 test 상위 5 그룹 점유율 ≤ 40%
하나라도 깨지면 그 층은 `descriptive_only=True`로 강등하고 결론에 쓰지 않는다.
...
| 외부 절제 ΔAUROC | 부호가 WebPhish와 동일 **그리고** 무작위 대조 대비 비율 > 2.0 |
