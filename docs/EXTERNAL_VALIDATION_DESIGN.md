...
**해석 프레임(최초 설계 시 사전 등록).** ΔAUROC_G가 0 근처로 나오는 것은 실패가 아니다. 그것은 **옛 G가 보던 차이가
...
실질적으로 중요한 최소 차이(SESOI)를 정하고 **CI 상한이 그보다 작다**는 형태의 근거가 있어야 한다.
그래서 상수를 하나 더 둔다. 다만 이 상수는 **개정(사후)** 항목이다. 기존 결과를 확인한 뒤, 비유의성을
효과 부재로 해석하는 문제를 수정하기 위해 AUROC 0.02를 기준으로 사후 재분석하였다(2026-09-09). 같은
캠페인 추정치와 CI가 이미 계산되어 있었으므로 사전 등록이 아니며, 이 판정은 **누출 이득의 단측 상한
...
**최초 설계(1차 실험 전)에 사전 등록한 항목과, 결과를 본 뒤 고친 개정(사후) 항목을 구분해 표시한다.**
아래에서 `개정(사후)`로 표시된 항목 — SESOI 0.02 판정 규칙(2026-09-09), 길이×경로 유무 매칭 cohort를
F-a의 주 판정으로 바꾼 결정(2026-09-09) — 은 사전 등록이 아니라 사후 재분석이다. 나머지 항목은 실행 전에
확정한 것이며 결과를 본 뒤 고치지 않는다.
...
- **경로 편향 플래그가 켜진 세트**에서는 **길이×경로 유무 매칭** cohort(`a_fixed_lenpath`)의 결과가
  F-a의 **주 판정**이고, 길이만 맞춘 일반 cohort는 보조로 내린다. 이 변경은 **개정(사후)** 항목이다
  (2026-09-09). 기존 결과에서 두 세트의 경로 편향 방향이 같다는 것을 확인한 뒤 도입했으므로 사전 등록이
  아니다. 매칭이 맞추는 것은 길이와 `path_depth >= 1`의 결합 분포뿐이며 경로 깊이 자체, 구분자 구성,
  쿼리 구조 전체를 맞추지는 않는다. 또한 매칭은 평가 표본 자체를 바꾸므로(v2 primary 15,480 → 7,260)
  매칭 전후 점수 차이를 경로 편향의 인과적 기여량으로 읽어서는 안 된다.
- **`path_lr`**은 `qrphish.urls.shape_features()`가 만드는 6차원 — 경로 유무, 경로 깊이, 그리고
  `/`, `.`, `-`, `?`의 개수 — 을 쓰는 로지스틱 회귀다. URL 어휘도 길이도
  보지 않으므로, 이 기준선이 높다는 것은 두 데이터셋이 경로 모양 축에서 같은 방향의 편향을 공유할 가능성을 보여 준다.
  전이 AUROC가 0.80 이상이면 그 층의 전이 성능은 경로 shortcut에 지배된다고 보고 `descriptive_only`로
  강등한다. 반대로 이 기준선이 낮아졌다고 경로 정보가 사라졌다고 읽어서도 안 된다. 0.5보다 뚜렷이 낮은
  값(예: v3 매칭 cohort의 0.394)은 관계의 방향이 뒤집혔을 가능성을 시사하며, 한 선형 기준선의 전이
  실패가 그 특징군 전체의 예측력 부재를 입증하지는 않는다.
...
**2026-09-09 개정(사후) (review_03 5절).** 아래 표가 개정 판정 규칙이며, 기존 결과를 본 뒤에 도입한 사후 재분석 규칙이다. 옛 표의 첫 줄("쌍체 CI가 0을 포함
...
따른 전체 AUROC 상승을 검출하지 못했다"까지만 쓴다. `negligible`이 나온 지표에 대해서는 "누출 허용에
따른 전체 AUROC 상승의 단측 상한을 0.02 아래로 한정했다"까지 쓸 수 있고, 그때에도 주장 범위는 그 지표가
실제로 덮는 집합(전체 T)까지다. "등가성을 보였다", "kit 암기 가능성을 배제했다"는 어느 지표에서도 쓰지
않는다.
