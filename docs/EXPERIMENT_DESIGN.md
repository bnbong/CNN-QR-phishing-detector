...
QR은 URL의 무손실·결정적 인코딩이다. 따라서 "QR 이미지에서 피싱을 분류할 수 있는가"는 정보이론적으로 자명하게 참이고(디코딩 후 텍스트 분류기를 쓰면 됨), 논문의 실제 질문은 두 가지다.

- **Q1 (복원 가능성)** 합성곱 귀납편향을 가진 모델이 *디코딩 없이* 모듈 격자에서 URL 어휘 신호를 얼마나 복원하는가. 정량 지표는 **텍스트 상한선 대비 갭**이다.
- **Q2 (공통 패턴)** 피싱 QR에 "공통 시각 패턴"이 존재한다는 주장은, **길이·버전·패딩 경계 같은 메타 신호를 모두 통제한 뒤에도** 성능이 우연 수준을 유의하게 넘는지로만 검증된다.

논문은 3층 구조로 쓴다.

| 층 | 내용 | 역할 |
|---|---|---|
| 상한선 | URL char n-gram TF-IDF LR, char-CNN | CNN이 이론적으로 도달 가능한 천장 |
| 하한선 | 버전 단독 LR, 길이 단독 LR, 바이트 히스토그램 LR | "메타 신호만으로 얼마나 되는가" |
| 본 실험 | 모듈 격자 CNN / MLP | 갭의 위치를 규정 |

**선언할 사실**: CNN 성능이 텍스트 상한선을 넘을 수 없다. 넘는다면 누출이다. 이 부등식을 sanity check로 상시 감시한다.
...
**추가 절제 `shuffle-pos`**: 층 내 전체 샘플에 **동일한 고정 순열**로 모듈 위치를 섞은 입력. CNN 성능이 이때 급락하고 MLP는 불변이라면 "CNN이 공간적 국소성을 실제로 쓴다"는 증거가 된다. 반대로 CNN이 그대로면 국소 패턴 주장은 성립하지 않는다. **Q2에 대한 가장 직접적인 검정이므로 필수 조건이다.**
...
- CI: 시드별 test에서 그룹 클러스터 부트스트랩 2,000회 → 시드 간 평균. 시드 간 표준편차도 별도 보고.
- 조건 간 비교는 **동일 split에서의 쌍체 차이** 분포로 한다.
- **사전 등록된 주 가설 목록**(아래 4개)에만 Holm 보정을 적용한다. 나머지는 탐색적(exploratory)으로 표기.
  ...
  - H2: 위 조건의 CNN AUROC < 텍스트 상한선 AUROC (갭 존재)
  - H3: `L-exact` AUROC < `L-none` AUROC (길이 아티팩트 크기)
...
```json
{
  ...
  "aggregate": { "auroc_mean": 0.0, "auroc_sd_across_seeds": 0.0,
                 "auroc_ci_pooled": [0.0,0.0], "f1_mean": 0.0, "acc_mean": 0.0 },
  "sanity": { "below_text_upper_bound": true, "label_shuffle_auroc": 0.5 },
...
- F4 층별 AUROC 막대 + CI, 텍스트 상한선과 셔플 바닥을 수평선으로 겹침
...
4. **char-CNN 상한선을 실제로 구현할 것인가, char n-gram LR만으로 충분한가?** 후자만으로도 논증은 성립하고 예산이 절약된다.
...
4. **상한선은 char n-gram LR만 필수.** char-CNN은 예산이 남을 때만 돌리는 선택 항목이다.
