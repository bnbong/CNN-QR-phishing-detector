...
> 보고 benign을 맨 도메인 코퍼스로 단정했는데, 경로 통계를 직접 세어 보니 반대다.
...
> 남는 편향축은 경로 유무가 아니라 출처 결합(인기 도메인 대 신고 URL)과 `www.` 접두다.
