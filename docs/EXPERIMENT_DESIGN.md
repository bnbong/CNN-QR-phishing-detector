...
이 문서는 내부 통제 실험의 설계와 변경 이력을 설명한다. 초기 계획에 해당하는 인터페이스 예시와 예산은 기록으로 보존하며, 실제 실행 절차는 `docs/RUNBOOK.md`, 결과는 `docs/RESULTS.md`를 기준으로 한다. 외부 수집과 전이, 캠페인 홀드아웃은 이후 구현 및 실행되었으며 `docs/EXTERNAL_VALIDATION_DESIGN.md`에서 다룬다.

---

## 0. 연구 명제와 논증 구조

QR은 URL의 무손실, 결정적 인코딩이다. 다만 인코딩이 문자열 정보를 보존한다는 사실만으로 "QR 이미지에서 피싱을 분류할 수 있는가"가 자동으로 풀리지는 않는다. 그 정보의 라벨 예측력과 제한된 CNN의 학습 가능성은 실험 문제다. 논문의 실제 질문은 아래 셋이다. 이 **RQ1 ~ RQ3가 유일한 기준 연구 질문**이며 14.1절, `docs/RESULTS.md`, `README.md`가 모두 같은 정의를 쓴다.

- **RQ1 (shortcut 통제 후 신호 존재)** 길이와 QR 버전, 패딩 경계의 클래스별 분포를 통제한 뒤에도 QR 모듈 격자에 피싱 여부를 구분하는 신호가 남는가. 검증은 그 조건에서 성능이 라벨 셔플 대조군을 유의하게 넘는지로만 한다.
- **RQ2 (디코딩 없는 신호 접근)** CNN 표현에서 URL의 어휘와 구조 속성을 선형으로 예측할 수 있는가. 피싱 분류 학습이 미학습 CNN보다 그 접근성을 높이는가. 프로브는 이 두 질문을 구분하며, 실제 분류 결정에 해당 속성이 사용되는지는 측정하지 않는다. 접근성 통과 목표는 v2 26/91, v3 13/114, v4 1/104이며 학습 이득은 v2의 경로 구조 두 목표에서만 확인되었다.
- **RQ3 (국소 패턴과 일반화)** 클래스와 연관된 국소 패턴이 있고, 그 패턴과 분류 성능이 새로운 도메인과 데이터셋에서도 재현되는가.
...
| 본 실험 | 모듈 격자 CNN / MLP | 차이의 위치를 규정 |

**참조 기준의 지위**: char 1~5 gram TF-IDF + Logistic Regression은 **하나의 강한 텍스트 분류기**일 뿐 Bayes 최적 분류기가 아니다. QR이 URL의 결정적 변환이라는 점에서 정보론적으로 디코딩된 URL에 QR보다 적은 정보가 담기지는 않지만, 특정 LR이 그 정보론적 상한을 구현하지는 못한다. 따라서 이 값을 "상한선"이라 부르지 않으며, **CNN이 이를 넘더라도 누출의 증거가 아니다**. 보고하는 것은 부호 있는 차이(`gap_to_decoded_text` = 참조 - CNN) 하나뿐이고, 누출 판정용 sanity check는 두지 않는다. sanity에는 라벨 셔플 AUROC만 남긴다.

---

## 1. 설계 비판과 결정

### 1.1 패딩 경계와 URL 길이의 관계

`qrphish.qrgen.build_data`는 데이터 뒤에 종단자, 바이트 정렬 비트, `0xEC`와 `0x11`의 교대 패딩을 추가한다. 버전과 바이트 모드가 고정되면 패딩 경계는 URL 바이트 길이에 따라 정해진다. 따라서 패딩 구조를 통해 길이에 관한 단서가 남을 수 있다. 주 조건인 v2~v4와 EC=L은 단일 오류정정 블록이므로 여러 블록의 인터리빙이 패딩을 확산시킨다고 설명하지 않는다.

버전 층화만으로는 층 안의 길이 분포 차이가 제거되지 않는다. 다만 클래스별 평균 길이가 다르다는 사실만으로 CNN이 패딩 경계를 학습하거나 거의 완벽하게 분류한다고 단정할 수는 없다. 길이 매칭과 음성 대조로 이를 평가한다.

**결정 (필수, 주 조건)**
1. **엄격 길이 매칭(exact-length matching)**: URL 바이트 길이를 1바이트 단위 버킷으로 쪼개고, 각 버킷에서 두 클래스 중 적은 쪽 개수 `min(n_ben, n_phi)`만큼 양쪽에서 샘플링한다. 결과 데이터셋은 **길이 주변분포가 클래스 간 완전히 동일**하고, 따라서 패딩 경계 분포도 동일하다. 이것이 주 조건 `L-exact`다.
...
2. **완화 매칭(`L-quantile`)**: 표본이 너무 줄면 5바이트 폭 버킷 매칭을 보조 조건으로 둔다. 이때 패딩 경계는 같은 버킷 안의 최대 4바이트 차이로 남으므로 "잔여 누출 있음"을 명시 보고한다.
3. **무통제(`L-none`)**: 길이 매칭 없이 버전 층화와 도메인 분리를 적용한다. `L-exact`와는 학습 및 평가 표본이 달라지므로 성능 차이를 길이의 인과적 기여량으로 해석하지 않는다.
4. **패딩 무작위화(`PAD-rand`)**: 표준 교대 패딩을 URL 해시 기반 의사난수 바이트로 바꾸고 오류정정 코드워드를 다시 계산한다. `L-none`에서 같은 URL을 비교하되, 패딩 위치만 수정한 개입은 아니라는 점을 명시한다. 원래 URL 바이트는 유지되지만 실제 디코더 호환성 전체를 검증한 실험은 아니다.
...
- v2(18 ~ 32바이트)와 v3(33 ~ 53바이트)는 benign이 다수, phishing이 소수 → 매칭 후 개수는 phishing이 결정.
...
최소 표본 규칙은 설정 파일에 명시하고 `reports/stratum_counts.json`으로 적용 결과를 확인한다. 실제로 v2, v3, v4는 주 분석 기준을 충족했고, v5+는 클래스당 266개로 제외되었다. 외부 등록 기관의 사전등록을 뜻하는 것은 아니다.

### 1.3 [높음] 버전 층 내에서 EC 레벨을 고정하는 것의 의미

주 실험은 EC=L로 고정한다. 오류정정 레벨이 바뀌면 같은 URL의 데이터 용량과 최소 버전이 달라질 수 있다. 같은 버전에서는 L의 데이터 용량이 더 크지만, 이것만으로 길이 누출의 크기가 최대라고 단정할 수는 없다. v3의 EC=M은 보조 조건으로 평가한다.

**결정**: 주 실험 EC=L. **강건성 확인용으로 v3 층 하나에 대해서만 EC=M 재실행**(`EC-M` 조건). EC 전체 격자 탐색은 하지 않는다(이번 실행 범위에서 제외).
...
### 1.5 수집 출처와 경로 분포의 편향

> **정정 (2026-09-09, review_03 4절).** 이 절의 전제 중 "benign은 대부분 맨 도메인"은 **틀렸다.**
> 설계 단계에서 `www.` 비율(benign 61% 대 phishing 13%)과 바이트 길이 중앙값(benign 32, phishing 38)만
> 보고 benign을 맨 도메인 자료로 판단했으나, 경로 통계를 직접 세어 보니 반대다.
> `reports/external/bias_stages.json` 기준으로 원본 WebPhish의 `path_depth>=1` 비율은
> benign 99.7% 대 phishing 69.8%이고, v2 층 길이 매칭 후에도 99.75% 대 44.3%다.
> 즉 WebPhish benign은 **경로를 가진 심층 페이지 코퍼스**이며 경로 보유율은 benign 쪽이 훨씬 높다.
> 아래 결정 자체(정규화 2종, `path+` 부분집합, 한계 절 명시)는 그대로 유효하지만,
> 편향의 **방향**을 "benign 짧고 경로 없음 / phishing 길고 경로 있음"으로 적은 서술은 전부 폐기한다.
> 남는 편향에는 정상 URL의 높은 경로 보유율, 수집 출처의 차이, `www.` 접두 비율 등이 있다.

benign은 인기 도메인 목록에서, phishing은 신고 기반 수집에서 왔다. 라벨과 출처가 사실상 결합되어 있다. 이것은 QR 실험의 결함이 아니라 **WebPhish 데이터셋 자체의 결함**이며, 어떤 인코딩을 써도 남는다. 숨기지 말고 정면으로 다룬다.

**결정**
- 정규화 조건 2종은 예정대로: `raw`(원문) / `norm`(소문자화 + 선행 `www.` 제거 + 말미 `/` 제거). `norm`은 `www.` 프록시(benign 61% vs phishing 13%)를 제거한다.
- **추가 조건 `path+`**: 경로 깊이 ≥1(호스트 뒤에 `/`+문자 존재)인 URL만 사용하는 부분집합. 클래스별 표본 수가 달라지므로 1.2 규칙에 따라 채택 여부를 카운트로 결정. 이는 경로가 있는 URL 부분집합의 결과로 보고하며 전체 결과보다 우월한 통제로 간주하지 않는다.
- 위협 타당성 절(Limitations)에 "benign 표집이 Alexa 상위이므로 결과는 '피싱 vs 인기 도메인' 구분이며 '피싱 vs 일반 웹' 구분이 아니다"를 명시. 외부 평가에서도 출처 편향이 유지될 수 있으므로 이를 별도로 진단한다.

### 1.6 [중간] 마스크 고정의 부작용, 그리고 언마스킹

자동 마스크는 고정된 생성 절차에서 URL에 따라 마스크를 선택한다. 선택 결과가 라벨과 연관될 가능성은 마스크 인덱스 단독 LR로 평가하며, 실제 결과는 우연 수준이었다. 자동 마스크에서 CNN 성능이 낮아진 이유로 표본 간 패턴 대응의 변화를 고려할 수 있지만, 필요한 함수가 정확히 8배로 늘어난다고 하거나 원인을 확정하지 않는다.
...
**추가 절제 `shuffle-pos`**: 모든 표본에 동일한 고정 순열을 적용해 데이터 모듈의 원래 인접 관계를 바꾼다. 입력 정보는 보존되며, 두 배치에서 모델을 각각 재학습한다. BitMLP도 위치를 사용할 수 있지만 고정 입력 순열에 대해 함수 표현 능력이 유지된다. 이 비교는 원래 배치가 해당 CNN의 학습에 유리한지 평가하며, 문자 복원이나 특정 특징의 실제 활용을 직접 검증하지는 않는다.

### 1.8 [중간] 통계 절차

- 시드 5개 × (그룹 분할 재생성 + 모델 초기화 + 매칭 서브샘플링 재추출). 서브샘플링도 시드에 연결해 재현한다. CI는 저장된 모델 예측에 조건부인 평가 불확실성이며, 재학습 과정 전체의 불확실성을 포함하지 않는다.
...
- **p값의 지위**: 본문 주 결과는 ΔAUROC 점추정과 클러스터 부트스트랩 CI다. 보고하는 p값은 CI를 역전시켜 정의한 **bootstrap-tail pseudo-p**이고, 영가설 아래에서 재표집한 정식 귀무분포 p값이 아니다. 표, 본문에서도 `p`가 아니라 `bootstrap-tail pseudo-p (Holm)`으로 표기하고 보조 지표로만 읽는다. H1, H4처럼 같은 test 행을 공유하는 쌍체 가설에는 **클러스터 단위 순열 검정 p값을 산출했다**이며, 그 값이 나오면 정식 p값 자리를 그것이 맡는다.
- **사전 지정한 주 가설 목록**(아래 4개)에만 Holm 보정을 적용한다. 나머지는 탐색적(exploratory)으로 표기.  - H1: `L-exact/feat-data/mask-fixed` CNN AUROC > 0.5 (라벨 셔플 대비). **라벨 셔플은 train+val에만 건다.** test는 원 라벨로 평가해야 "라벨과 무관하게 학습된 모델이 실제 라벨을 맞히지 못한다"를 보는 것이 된다. test까지 섞으면 섞인 라벨을 맞히는지를 보게 되어 검정이 무의미해진다.  - H2: 위 조건의 CNN AUROC < 디코딩 텍스트 참조 기준 AUROC (갭 존재). 참조 기준은 강한 텍스트 베이스라인이지 천장이 아니므로, 이 검정은 "차이가 남는가"를 묻는 것이지 누출 판정이 아니다.  - H3: `L-exact` AUROC < `L-none` AUROC (길이 아티팩트 크기). **단서**: `L-none`과 `L-exact`는 표본 수, 클래스 구성, URL 집합이 함께 달라지므로(예: v2는 exact N=12,118, none N=16,627) 이 차이는 길이 신호만 더한 순수한 개입이 아니라 훈련 및 평가 집합 구성까지 동시에 바꾼 비교다. 쌍체 부트스트랩도 성립하지 않는다. 길이 아티팩트의 **인과적 절제**로는 같은 URL에서 패딩과 이에 따른 오류정정 코드를 바꾸는 `PAD-rand` 쪽이 더 가깝다.  - H4: `shuffle-pos` AUROC < 원래 배치 AUROC (공간 구조 사용 여부)
...
- **잔여 비트(remainder bits)**: 데이터 모듈 수가 총 코드워드 비트 수의 배수가 아닌 버전이 있다. 실측(EC=L)으로 **v2 ~ v6은 7비트, v1과 v7 ~ v13은 0비트**다. 더 높은 버전은 규격에 따라 잔여 비트 수가 다르다. 이 비트들은 마스킹 전에 0으로 배치된 뒤 마스크만 적용되므로 **클래스 정보가 없다**. 역매핑에서 `remainder`로 태깅하고, `feat-data` 조건에서 데이터 마스크에 포함시키되 해석에서는 제외한다.
...
### 계산 예산 아래는 초기 실행 전 예산 추정이며 실측 실행 시간이 아니다. 실제 조건 범위는 `configs/matrix.yaml`을 따른다.
...
## 8. 결과 JSON 스키마 예시 (`artifacts/{cond}/{stratum}/results.json`)
...
## 10. 초기 Colab 노트북 흐름 (`notebooks/colab_run.ipynb`)
...
아래는 초기 번호 계획이다. 현재 F1~F7의 번호와 설명은 `reports/figures/captions.md`를 따른다.
...
- F5 `shuffle-pos` 대 원래 배치 (CNN vs MLP 2×2) - H4
...
| # | 쟁점 | 결정 | 등급 |
|---|---|---|---|
| 1 | 패딩 바이트 길이 누출 | 주 조건은 1바이트 버킷 **엄격 길이 매칭**. `L-none`/`L-quantile`은 절제로만. `PAD-rand`로 인과 검증 | 필수 |
| 2 | 층별 최소 표본 | 클래스당 ≥1000 primary / ≥300 secondary / <300 폐기. 데이터 보기 전 사전 지정 | 필수 |
...
3. **`mask-off`는 해석 부록 전용.** 본문 성능 표는 `mask-fixed`만 쓴다. 언마스킹 격자의 성능 수치는 "표현 학습 보조 결과"로 부록에 따로 적는다.
...
| 새 연구 질문 | 현재 상태 |
|---|---|
| **RQ1.** 길이와 버전, 패딩 등의 shortcut을 통제한 후에도 QR 모듈 격자에 피싱 여부를 구분하는 신호가 존재하는가? | 주 실험에서 판별력 확인 |
| **RQ2.** CNN은 명시적 QR decoding 없이 QR의 공간 배치를 이용해 URL 유래 내용과 순서 신호(URL-derived content/order signal)에 접근하는가? | 일부 속성의 선형 접근성 확인. 학습 이득과 실제 활용은 구분 |
| **RQ3.** 그 signal은 여러 phishing QR에서 반복되는 local visual motif로 나타나며, 새로운 도메인, 데이터셋에서도 재현되는가? | 외부 전이와 motif 재현성 평가 완료. 보편적 패턴은 미입증 |
...
| 클래스와 연관된 국소 패치 빈도 분포(클래스와 연관된 국소 패치 빈도 분포)가 존재한다 | 지지 (`RESULTS.md` 7절) |
| 전체 비트 조성으로 설명되지 않는 국소 공간 의존성이 존재한다 | 모듈 셔플에서 성능 저하 확인. 코드워드 셔플은 URL 바이트 조성을 보존하지 않음 |
| CNN 내부에서 URL 문자/n-gram 자체가 복원된다 | 사용한 선형 프로브와 판정 기준으로는 복원 증거를 확보하지 못했다. 선형으로 읽히는 목표는 v2 26/91, v3 13/114, v4 1/104이고, 학습에 의한 접근성 향상은 v2의 경로 구조 두 목표(`has_path`, `path_depth`)에서만 확인되었다 (`RESULTS.md` 5-B절) |
| 이 패턴이 다른 피싱 데이터에서도 일반화된다 | 약한 분류 전이 확인. 개별 motif 재현성은 제한적 |
...
**C. 인과 motif 절제 (RQ3) - motif 표적 중심 비트 개입.** B가 찾은 상위 phishing motif와 **매칭된 3×3 창의 중심 모듈 하나만 뒤집고** 원본 대비 ΔAUROC와 평균 로짓 변화를 잰다. 이 개입의 이름은 motif 표적 중심 비트 개입(motif-targeted center-bit intervention)이며, "motif 뒤집기(flipping)"라고 부르지 않는다. 창 전체를 바꾸는 것이 아니라 창당 모듈 하나를 바꾸는 것이기 때문이다. 대조는 둘이다. 상위 benign motif에 같은 개입, 그리고 **각 샘플에서 phishing motif가 맞은 개수와 똑같은 수**의 무작위 데이터 모듈 뒤집기(5회 반복 평균). 후자가 "뒤집은 모듈 수" 자체의 효과를 상쇄한다. phishing과 benign은 매칭 개수가 다르므로 무작위 대조도 둘로 나눈다. `random`은 phishing 매칭 수에, `random_benign_matched`는 benign 매칭 수에 맞춘다. 매칭은 원본 격자 한 벌에서 모두 계산한 뒤 한꺼번에 뒤집는다. 하나씩 순차로 뒤집으면 이웃 창의 id가 바뀌어 결과가 순서에 의존한다. 원본과 개입본이 같은 표본을 공유하므로 비교는 전부 쌍체 그룹 부트스트랩이다. 중심 비트 개입은 오류정정 관계를 깨뜨릴 수 있다. 실제 디코딩 실패를 검증한 것은 아니며 모델 입력에 대한 개입으로 해석한다.

현재 구현에는 개수 대조와 위치 및 밀도 대조가 모두 있다. 위치 및 밀도 대조는 같은 사분면과 3×3 창의 검은 모듈 수 차이 1개 이내에서 후보를 고르며 실제 motif 중심을 제외한다. 후보가 부족하면 밀도 제약, 이어서 사분면 제약을 완화하고 그 횟수를 기록한다. 따라서 완전한 위치 및 밀도 일치를 보장하지 않는다.

**D. 어휘 프로브 (RQ2).** SmallCNN의 GAP 이후(global average pooling 출력) 128차원 임베딩을 freeze하고 URL 어휘 속성을 선형으로 읽어낸다. 목표는 train URL 상위 char 3-gram 100개, 피싱 문헌의 키워드 16개(`login`, `verify`, `.com` 등), 구조 특성(path, query, 숫자, 하이픈, IP 호스트), 연속값(숫자 비율, 특수문자 비율, 점 개수, 경로 깊이, 서브도메인 수)이다. n-gram 목록은 **라벨을 보지 않고** train URL 빈도로만 고른다. 라벨을 보고 고르면 목표 선택 자체가 누출이다. 이진 목표는 AUROC, 연속 목표는 R²로 잰다.

기준선은 둘이다. **그룹 단위 셔플**(각 그룹에 다른 그룹을 도너로 배정해 목표값을 재배정한다. 행 단위로 섞으면 그룹 안에서 목표가 거의 상수인 경우 정보가 남아 기준선이 부풀려진다)과 **무작위 초기화 CNN**(체크포인트에 기록된 같은 arch, 학습만 안 한 모델). 통과 판정은 시드마다 따로 하고, 모든 시드에서 `CI 하한 > max(셔플 CI 상한, 무작위 초기화 CI 상한)`일 때만 두 기준을 모두 통과한 것으로 센다. 두 기준선을 CI 수준에서 모두 넘어야 하며, 일부 시드에서 표본 부족으로 건너뛴 목표는 전 시드 일치를 확인할 수 없으므로 통과로 세지 않는다. n-gram 목표 유니버스는 층에서 한 번만 고른다. 시드마다 다시 고르면 "모든 시드에서 두 기준을 통과"가 같은 대상을 두고 내린 판정이 아니게 된다. 라벨 프로브(`label:phishing`)는 임베딩이 애초에 라벨을 선형 분리하도록 학습됐으므로 참고 상한선일 뿐 어휘 증거가 아니며 요약 카운트에서 뺀다. 프로브가 통과해도 "문자를 복원했다"가 아니라 "그 속성이 임베딩에서 선형으로 접근 가능하다"는 뜻이다.
...
**판정 규칙 요약.** 주 가설 H1 ~ H4의 판정은 쌍체 ΔAUROC의 95% 양측 백분위 CI가 0을 배제하는지로 한다. p값은 그 CI를 역전시켜 정의하고(CI가 0을 배제하는 가장 작은 α) Holm 보정을 걸되, 이름은 `bootstrap-tail pseudo-p (Holm)`으로 쓴다. 영가설 아래에서 재표집한 정식 p값이 아니므로 정식 가설 검정 표에서 빼고 부록 성격의 참고값으로만 둔다. H1, H4는 클러스터 순열 p를, H2는 효과 추정치와 CI를 주 보고 지표로 삼는다. H1과 H4의 클러스터 순열 검정은 실행되었다. motif enrichment는 5시드 부호 일치 + held-out test 부호 일치 + 풀링 CI 0 배제 + 최소 출현율. 프로브는 전 시드에서 두 기준선 CI 상한 초과. 절제는 쌍체 CI가 0을 제외하는 시드 수로 보고한다.
...
| **F. 외부 검증** | WebPhish 학습 → 별도 PhishTank/OpenPhish phishing + Tranco/Common Crawl benign으로 평가 | 출처가 다른 데이터에서 전이 범위 평가 |
| **G. Campaign split** | eTLD+1뿐 아니라 URL token, path template의 MinHash 클러스터 단위 holdout | 정의한 템플릿 중복 허용의 영향 평가 |
| **H. 실제 QR 조건** | auto-mask, 여러 EC, 회전, 블러, 원근, JPEG, 인쇄 시뮬레이션 | 실제 "시각 QR"로 주장 확장 |

현재 F-a와 F-b, G는 실행되었다. F-c와 F-d, E, 실제 촬영 조건 H는 실행하지 않았다. 자동 마스크와 v3의 EC=M은 내부 보조 실험에 포함된다. 외부 검증과 캠페인 분리로 출처 편향이나 모든 템플릿 누출이 제거되었다고 해석하지 않는다.