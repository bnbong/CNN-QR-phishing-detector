...
층별 표본과 그룹 수를 아래에 정리했다. `n_total`은 결과 집계에 기록된 층별 전체 표본 수다. train / validation / test 수는 그룹 분리와 분할별 길이 매칭을 모두 거친 뒤 시드 0에서 실제로 학습 및 평가에 쓰인 행 수다. 두 수를 구분한다. 그룹 수는 길이 매칭 전 그룹 분리 단계에서 센 eTLD+1 수다.

| 층 | `n_total` (길이 매칭 후) | 시드 0 train | 시드 0 validation | 시드 0 test | 전체 eTLD+1 그룹 수 | train / val / test 그룹 수 |
|---|---|---|---|---|---|---|
| v2 | 12,118 | 8,502 | 1,840 | 1,776 | 9,558 | 6,750 / 1,404 / 1,404 |
| v3 | 9,178 | 6,496 | 1,318 | 1,364 | 6,198 | 4,437 / 881 / 880 |
| v4 | 2,714 | 1,932 | 382 | 400 | 2,035 | 1,424 / 305 / 306 |
...
| 시드 0 train / validation / test 행 수 재확인 | `data/webphish.csv`와 `configs/base.yaml`에 `qrphish.runner._prepare_frame(cfg, stratum, seed=0)` 적용. 2026-09-11 재계산했으며 모델 학습은 수행하지 않음 |
