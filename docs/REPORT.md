...
QR은 URL의 무손실·결정적 인코딩이다. 그러니 "QR 이미지로 피싱을 분류할 수 있는가"는 정보이론적으로
...
개별 URL n-gram이나 키워드가 선형으로 읽힌다는 증거도, 학습이 그 접근성을 높였다는 증거도 확보하지
못했다. 전 시드에서 셔플 기준선을 넘은 목표는 v2의 `has_path`와 `path_depth` 둘뿐이고 둘 다 어휘가
...
뒷부분: 약하다). 정리하면 분포 수준의 판별 신호는 다른 데이터셋에서도 바닥선 위에 남지만 개별 motif의
...
**자체 정정 두 건.** 이전 판본은 (a) motif 표적 개입이 무작위 개입보다 큰 성능 저하를 낳는다고, (b) v2에서
어휘 목표의 절반가량이 선형으로 접근 가능하다고 적었다. 외부 리뷰(review_03 2·3·6절)가 지적한 절차
문제(시드 통합 motif 선정, 무작위 대조만 로짓을 평균한 뒤 AUROC 계산, 시드 평균 CI로 근사한 프로브
판정)를 코드에서 고쳐 재실행하자 두 결론 모두 사라졌다. 4.6과 4.7에 정정 전후를 함께 싣는다. 결론이
어느 통합·대조 절차에 얼마나 의존했는지를 보여 주는 기록이므로 지우지 않고 남긴다.
...
제시한다. 이 연구의 기여는 접근의 신규성이 아니라 **평가의 분해**다.
...
- H1 ~ H4의 판정은 쌍체 ΔAUROC의 95% CI가 0을 배제하는지로 하고 CI를 역전시켜 정의한 p값에 Holm
  ...
  때문이다. 쌍체 예측이 있는 H1·H4에는 그룹 단위 교환 클러스터 순열 검정의 정식 p값을 함께 낸다.
...
- `bootstrap-tail pseudo-p`는 이름을 바꾼 것만으로 정식 p값의 성질이 생기지 않는다. Holm 보정도 그 문제를
  해결해 주지 않는다.
...
위 수치는 시드별 정확 판정이다. 이전 판본의 ① 수치(v2 45/91 등)는 시드 평균 CI로 근사한 값이었고,
시드마다 따로 판정해 전 시드 통과를 요구하면 v2 2 / 91, v3 0 / 114, v4 0 / 104로 줄어든다. 통과한 둘
(`has_path` AUROC 0.889, `path_depth` R² 0.245)은 어휘가 아니라 경로 구조 특징이다. 둘 다 미학습 기준선까지
넘어 ②도 통과한다. 개별 n-gram·키워드 중 ①을 통과한 것은 어느 층에도 없다.

시드 평균만 보면 v2 `ngram:jp.` 0.909, v3 `keyword:http` 0.862처럼 높은 목표가 여럿이지만 통과 시드가
0개다. 시드 간 변동이 커서 어떤 시드에서는 셔플 CI 상한을 넘지 못한다. 미학습 기준선의 역할도 그대로다.
...
정리하면 사용한 선형 프로브와 판정 기준으로는 어휘 정보의 선형 접근성도, 학습에 의한 접근성 향상도
확인하지 못했다. 확인된 것은 경로 구조 특징 둘뿐이며 이는 4.10의 경로 shortcut과 같은 축을 가리킨다.
"복원이 기각됐다"고 쓰지 않는다. 비선형 프로브나 문자 시퀀스 디코더는 시도하지 않았다.
...
(2.2절) 경로 유무·깊이·구분자 비율만 쓰는 path-shape LR 기준선과 길이·경로를 함께 맞춘
**`a_fixed_lenpath` cohort**를 넣었고, 후자가 주 판정이다. 길이만 맞춘 cohort는
...
유무·깊이·구분자 비율 세 가지만으로 전이 점수의 대부분이 설명된다는 뜻이므로 이 숫자를 "QR 공간 신호의
전이"로 읽으면 안 된다.

경로까지 맞춘 주 cohort에서 CNN은 0.649 [0.626, 0.670] / 0.639 [0.609, 0.667] / 0.677 [0.639, 0.724]이고
순열 바닥선은 0.530 / 0.537 / 0.558, 행 단위 순열 바닥선은 0.506 / 0.505 / 0.506이다. CI 하한이 세 층 모두
두 바닥선 위다. 같은 cohort에서 path-shape LR은 0.509 / 0.394 / 0.542로 무너지고 CNN 대비 쌍체 Δ가
+0.140 / +0.244 / +0.135로 뒤집힌다. 판정은 v2 `partial_collapse`, v3·v4 `reproduced_weak`다. 내부 성능
대비 하락폭은 −0.223 / −0.175 / −0.128로 길이만 맞췄을 때보다 커졌다.
...
motif-hist LR(3×3)은 0.576 / 0.559 / 0.591로 가장 낮다. 공간적일수록 덜 전이된다.
...
민감도와 강건성 조건은 결과를 흔들지 않는다. Tranco 조인을 끈 unranked Common Crawl benign은 주
...
등가성 형태로 판정한다. CI 상한이 0.02 미만이면 `negligible`, CI가 0을 포함하되 상한이 0.02 이상이면
...
CNN이 특정 문자열을 이용하는지 여부도 현재 분석으로는 확정할 수 없다. 어휘 프로브가 답한 것은 "어휘
정보의 선형 접근성도, 학습에 의한 접근성 향상도 확인하지 못했다"이지 "특정 문자열을 쓰지 않는다"가 아니다.
...
2. **신호 층위의 견고성 차이.** 전이되는 몫이 주로 문자 순서·조성이라면, 그것을 직접 세는 표현이 공간
   ...
   (0.559 ~ 0.591) 전이된 것이 이 방향과 맞는다. 공간적일수록 덜 전이된다.
...
판정하느냐 시드별로 판정하느냐에서, 외부 전이는 길이만 맞추느냐 경로까지 맞추느냐에서 답이 갈렸다.
...
11. **전이 판정이 cohort 정의에 민감하다.** 같은 데이터에서 길이만 맞추면 0.744 / 0.702 / 0.693,
    경로까지 맞추면 0.649 / 0.639 / 0.677이다. 어느 쪽을 주 판정으로 삼느냐가 결론의 크기를 바꾸므로
    사전에 경로까지 맞춘 쪽으로 정했다. 경로 편향 자체를 제거한 것은 아니고 그 몫을 떼어냈을 뿐이다.
12. **통계 절차의 한계.** `pseudo-p`는 정식 p값이 아니고 Holm 보정이 그 문제를 해결하지 않는다. 외부
    바닥선의 순열은 양성 총수를 보존하지 않는 그룹 블록 교환 근사이며, 행 단위 순열 바닥선을
    대조로 함께 싣는다.
13. **결론 두 개를 자체 정정했다.** motif의 인과 기여(4.6)와 어휘 정보의 선형 접근성(4.7)은 이전 판본에서
    지지된다고 적었으나 절차를 고치자 사라졌다. 남은 결론들도 같은 종류의 절차 의존성을 가질 수 있다.
...
길이와 경로를 함께 맞춘 cohort를 주 판정으로 삼았고, 거기서 zero-shot 전이 AUROC는 0.649 / 0.639 / 0.677이다.
